...
export function QuestionItem({
	...
	return (
		<button
			...
			disabled={disabled}
			className={cn(
				"bg-white rounded-4xl px-4 py-2 text-left transition-colors shadow-sm",
				disabled ? "opacity-40" : "hover:bg-gray-50",
			)}
		>
			<span className="typo-headline-1 font-semibold text-dnd-label-normal text-[18px]">
				{question.content}
			</span>
		</button>
	);
...
function RefreshQuestionsButton({ insightId }: { insightId: number }) {
	...
	return (
		<div className="flex flex-col gap-4 pt-4 relative group items-start">
			<button
				type="button"
				className="bg-white border border-dnd-line-strong rounded-lg py-2 px-3 shadow-sm hover:bg-gray-50 transition-colors disabled:opacity-50"
			...
			<div className="absolute top-full mt-2 left-0 right-0 z-10 bg-dnd-label-normal backdrop-blur-md rounded-lg p-2.5 text-dnd-bg-normal typo-body-2 text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto">
				<div className="absolute -top-1.5 left-4 w-3 h-3 bg-dnd-label-normal rotate-45 transform" />
