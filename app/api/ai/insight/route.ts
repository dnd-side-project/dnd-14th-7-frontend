...
function createPrompt(memo: string) {
	return `
다음 사용자의 업무·학습·경험 메모를 Aha!ve 서비스의 인사이트 데이터로 변환해줘.
반드시 마크다운 코드블록 없이 순수 JSON만 출력해.
부가 설명은 절대 포함하지 마.

목표:
- 단순 요약이 아니라 사용자가 겪은 상황, 배운 점, 다음에 적용할 원칙을 선명하게 정리
- 개발, 디자인, 기획, 기타 직군 어디에도 적용될 수 있는 실무적인 표현 사용
- 질문은 사용자의 경험을 더 깊게 회고하고 다음 행동으로 연결하도록 설계

요구사항:
- title: 핵심 경험을 담은 짧은 제목, 20자 이내
- insight: 메모를 바탕으로 정리한 핵심 상황·배운 점·적용 원칙을 한 문장으로 작성
- tags: 직군/상황/핵심 키워드 중심으로 3개
- questions: 사용자가 더 깊게 생각해볼 회고·적용·구체화 질문 3개

출력 형식:
{
  "title": "로그 전략의 필요성",
  "insight": "디버깅이 어려웠던 경험은 문제 해결 속도를 높이기 위해 작업 초반부터 관찰 가능한 로그 기준을 세워야 한다는 교훈으로 이어진다.",
  "tags": ["개발", "디버깅", "로그"],
  "questions": ["이번 경험에서 문제 원인을 늦게 찾게 만든 가장 큰 가정은 무엇이었나요?", "다음 작업에서 같은 문제를 줄이기 위해 먼저 확인할 기준은 무엇인가요?", "이 경험을 팀원에게 공유한다면 어떤 원칙으로 정리할 수 있나요?"]
}
